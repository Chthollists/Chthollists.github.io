...
* 在`themes\yilia\layout\_partial\footer.ejs`中，在`<div class="footer-right">`标签内添加
...
* 解决办法：
  ...
  * 修改不蒜子统计的调用语句
    ...
    * 原来是从不蒜子统计的服务器上调用`busuanzi.pure.mini.js`
...
* 获取外链
  ...
  > 注意：要获取多个音乐的外链可以将其添加到歌单、电台中，获取外链，auto 表示是否自动播放
  >
  > ```html
...
* 引入播放器代码
  ...
  * 通过设置`marginheight="0" width=330 height=110` 来改变大小，美化界面
...
* 原因推测
  ...
  > 是由于开启了IPv6后使用了google的`IPv6 DNS`，音乐资源网站IP解析会直接错误解析到本地`127.0.0.1`。

* [解决办法](https://blog.csdn.net/hubaoquanu/article/details/79084193)：均无效

  * 添加以下内容到`hosts`文件`C:\Windows\System32\drivers\etc\hosts`
  ...
  * 使用国内的`IPv6DNS: 240c::6666`，此时无法访问Google和Youtube
...
* 安装加密插件Encrypt
  ...
  > 找到博客根目录下的`package.json`，最下面添加，注意：添加时要记得给前一项加上`,`逗号
  >
  > ```yml
